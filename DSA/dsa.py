...
def generate_dsa(N, p, q, g, x, k, msg):
    """
    ...
    while s == 0:
        r = pow(g, k, p) % q
        while r == 0:
            k = random.randint(1, q) # Sprawdzić, czy możemy w ten sposób generować k oraz czy przechowywać tę informację
            r = pow(g, k, p) % q
        sha_dig = SHA.sha384.sha384(msg)
        n = min([N, len(sha_dig)])
        dig = int(sha_dig[:n], 2)
        s = (pow(k, -1, q) * (dig + x * r)) % q

    return [r, s]


def verify_dsa(N, p, q, g, y, sign, msg1):
    """
